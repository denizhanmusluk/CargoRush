...
public class AquariumCar : Stand
{
    ...
    {
        fishCountText.text = (cannedCount).ToString() + "/" + (fishCountTotal).ToString();
    ...
    {
        _CollectProduct.collectActive = false;
    ...
    {
        while (cannedCount < fishCountTotal)
        {

            cannedCount++;
            GameObject icedFish = Instantiate(productsPrefab.gameObject, transform.position, transform.rotation);
            ...
            float deltaY = 0;
            deltaY = (icedCollectionList.Count - 1) / currentCar.GetComponent<FishCar>().fishPosTR.Length;
            Transform targetTR = currentCar.GetComponent<FishCar>().fishPosTR[(icedCollectionList.Count - 1) % currentCar.GetComponent<FishCar>().fishPosTR.Length];
            Vector3 dropPos = targetTR.position + new Vector3(0, deltaY * 0.3f, 0);
            icedFish.transform.position = dropPos;
            icedFish.transform.rotation = targetTR.transform.rotation;
            icedFish.transform.parent = targetTR;
