...
public class SushiCooker : Stand
{
    ...
    IEnumerator CannedCreator()
    {
        ...
        if (droppedCollectionList.Count > 0 && cannedCount < sushiPosTR.Length)
        {
            MachineActive();
        }

        //float waitTime = 3f;
        while (droppedCollectionList.Count > 0 && cannedCount < sushiPosTR.Length)
        {
            //creatingActive = true;
            machineActive = true;
            cannedCount++;
